...
class IOFile():
    dir = 'dataset/data/'
    # dir = 'dataset/data200/'
    # dir = 'dataset/pressure0.6_data/'
    # dir = 'dataset/pressure0.3-t8926-n135-m35/'
    # dir = 'dataset/pressure0.3-t200-n100-m20/'
    demand = dir + 'demand.csv'
    qos = dir + 'qos.csv'
    bandwidth = dir + 'site_bandwidth.csv'
    config = dir + 'config.ini'
    output = dir + 'solution.txt'
...
class ServerSeriesPlot(Plot):  # x: time  y: many client bandwidth height. P.S. only for one server
    ...
    def plot(self):
        ...
        sep_idx = int(len(idx) * 0.0)  # 原来是0.8
        for label, height in zip(self.labels, np.array(self.heights)[:, idx]):
            ...
            self.bottom += height
        plt.ylim(top=max(self.bottom) * 1.2 + 2000)
        ...
        for x, y, label in zip(self.time[sep_idx:], self.bottom[sep_idx:], time_str[sep_idx:]):
            plt.text(x, y + 0.3 * max(self.bottom), f'{label} {y}', ha='center', va='bottom', rotation=70)
...
if __name__ == '__main__':
    ...
    get_input_data()
    if len(sys.argv) == 1:
        gauge_time('bash build_and_run.sh')
    else:
