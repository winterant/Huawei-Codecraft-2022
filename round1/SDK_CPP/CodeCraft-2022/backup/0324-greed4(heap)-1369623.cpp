...
public:
    ...
    void greed4()
    {
        ...
        /*************************** 每个边缘节点选出5%并分配流量 **************************************/
        // 统计每个时间片的总需求量，然后从大到小排序，优先处理需求量大的时间片
        vector<int64_t> sumDemandOfMoment(numMoments);   // 统计每个时刻的总需求量
        priority_queue<PLI> Qmoments;                    // 堆顶是请求量最大的时间片
        vector<bool> inQmoments(numMoments);             // 标记时间片是否已入队列，防止堆内重复时间片
        vector<PLI> sortedSumDemandOfMoment(numMoments); // debug
        for (int t = 0; t < numMoments; ++t)
        {
            sumDemandOfMoment[t] = 0;
            for (int u = 0; u < numUsers; ++u)
                sumDemandOfMoment[t] += demandInfo[t][u];
            Qmoments.push(PLI(sumDemandOfMoment[t], t));
            inQmoments[t] = true;
            sortedSumDemandOfMoment[t] = PLI(sumDemandOfMoment[t], t);
        }
        sort(sortedSumDemandOfMoment.begin(), sortedSumDemandOfMoment.end(), greater<PLI>());
        // sort(sumDemandOfMoment.begin(), sumDemandOfMoment.end(), greater<PLI>()); // 时间片按需求量从大到小排序

        // 遍历时间片，维护5%堆
        priority_queue<PLI, vector<PLI>, greater<PLI>> QSite5Moments[numSites]; // 每个边缘节点一个堆，维护5%
        ...
        vector<vector<bool>> canBe5(numMoments, vector<bool>(numSites, true)); // 标记点是否还能作为5%
        while (!Qmoments.empty())
        {
            int moment = Qmoments.top().second; // 取出请求最大的时间片
            Qmoments.pop();
            inQmoments[moment] = false;
            // cout << (++counter) << " moment " << moment << "  sum demand: " << sumDemandOfMoment[moment] << endl;
            // 贪心规划时间片moment；策略1：偏远边缘节点优先拉满；策略2：各自拉满找出第一个，残余请求拉满找第2个，类推；
            for (int &sid : sortedSiteIds)
            {
                if (!canBe5[moment][sid])
                    continue;
                // 将sid带宽拉满
                int bandwidth = siteList.getBandwidth(sid);
                for (int &uid : siteConnUserIds[sid])
                {
                    int flow = min(demandInfo[moment][uid], bandwidth);
                    if (flow == 0)
                        continue;
                    demandInfo[moment][uid] -= flow;
                    bandwidth -= flow;
                    sitesFlow[moment][sid] += flow;
                    flowOfUser2Site[moment][sid][uid] += flow;
                }
            }
            // 调整堆结构
            for (int &sid : sortedSiteIds)
            {
                if (!canBe5[moment][sid])
                    continue;
                // 拉满后放入堆
                if (sitesFlow[moment][sid] > 0)
                {
                    QSite5Moments[sid].push(PLI(sitesFlow[moment][sid], moment));
                    // cout << " select  t=" << moment << " s=" << sid << "  site flow=" << sitesFlow[moment][sid] << endl;
                }
                // 堆满了，把最小的时刻边缘拿出来，把请求退回去，并在下一轮重新规划时间片
                if ((int)QSite5Moments[sid].size() > num5moments)
                {
                    int backFlowMoment = QSite5Moments[sid].top().second;
                    // cout << " --------- back flow t=" << backFlowMoment << " s=" << sid << "  site flow=" << QSite5Moments[sid].top().first << endl;
                    QSite5Moments[sid].pop();
                    canBe5[backFlowMoment][sid] = false; // 标记该点再也无法成为5%
                    if (!inQmoments[backFlowMoment])
                    {
                        Qmoments.push(PLI(sumDemandOfMoment[backFlowMoment], backFlowMoment)); // 回退过请求的时间片，打回去重新规划
                        inQmoments[backFlowMoment] = true;
                    }
                    for (int &uid : siteConnUserIds[sid])
                    {
                        int backFlow = flowOfUser2Site[backFlowMoment][sid][uid]; // 已经分配的请求退回去
                        if (backFlow == 0)
                            continue;
                        demandInfo[backFlowMoment][uid] += backFlow;
                        sitesFlow[backFlowMoment][sid] -= backFlow;
                        flowOfUser2Site[backFlowMoment][sid][uid] -= backFlow;
                    }
                    // cout << "      After back flow, site flow is " << sitesFlow[backFlowMoment][sid] << endl;
                }
            }
            /*
            cout << " ------------------------------- View site flow ------------------------------- " << endl;
            cout << "Qsize";
            for (int s = 0; s < 50; ++s)
                if (siteConnUserIds[s].size() > 0)
                    printf("%6d ", QSite5Moments[s].size());
            printf("\n");
            cout << "site ";
            for (int s = 0; s < 50; ++s)
                if (siteConnUserIds[s].size() > 0)
            ...
            printf("\n");
            int cc = numMoments;
            for (int i = numMoments - 1; i >= 0; i--)
            {
                int64_t sss = 0;
                for (int s = 0; s < numSites; ++s)
                    sss += sitesFlow[sortedSumDemandOfMoment[i].second][s];
                if (sss > 0)
                {
                    cc = i + 2;
                    break;
            ...
            cout << "cc=" << cc << endl;
            for (auto &p : sortedSumDemandOfMoment)
            {
                // if (cc-- == 0) break;
                int t = p.second;
                printf("t=%2d:", t);
        ...
        /*
        cout << " ----------------------- Greed Debug ------------------------ " << endl;
        cout << "队列大小:" << endl;
        for (int s = 0; s < numSites; ++s)
            cout << QSite5Moments[s].size() << " ";
        cout << endl;
        int64_t flowOf5 = 0;
        for (int t = 0; t < numMoments; ++t)
            for (int s = 0; s < numSites; ++s)
                flowOf5 += sitesFlow[t][s];
        cout << " sum of flow 5% sites: " << flowOf5 << endl;

        cout << " Remain demand:" << endl;
        int64_t sumsum = 0;
        for (int t = 0; t < numMoments; ++t)
        {
            int64_t sumSiteDemand = 0;

            for (int u = 0; u < numUsers; ++u)
                sumSiteDemand += demandInfo[t][u];
            sumsum += sumSiteDemand;
            cout << sumSiteDemand << " ";
        }
        cout << endl;
        cout << " total remain demand " << sumsum << endl;
        cout << " -------------------------------------------------------- " << endl;
        */
    ...
    void buildGraph(int64_t defaultCost = 1)
    {
        ...
        for (int t = 0; t < numMoments; ++t)
        {
            ...
            for (int &s : sortedSiteIds)
            {
                ...
                if (!is5[t][s]) // todo
                {
                    vector<int64_t> flows({(upperDiv(siteSum95Flow[s], num95moments) + 1) / 2, // 前95均值/2
