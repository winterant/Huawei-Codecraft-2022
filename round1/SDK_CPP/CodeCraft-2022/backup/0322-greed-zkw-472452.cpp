...
public:
    ...
    {
        // 统计每个边缘节点可连接的用户数量并排序
        static vector<PII> siteConnNumOfUsers(numSites);
        for (int s = 0; s < numSites; ++s)
        {
            siteConnNumOfUsers[s].first = siteConnUsers[s].size();
            siteConnNumOfUsers[s].second = s;
        }
        sort(siteConnNumOfUsers.begin(), siteConnNumOfUsers.end());
        // 将每个边缘节点选出5%个时间点满载
        auto demandTemp = demand.cloneDemand();
        for (auto &p : siteConnNumOfUsers)
        {
            int siteId = p.second;
            // 统计当前边缘节点每个时间片的可满载值
    ...
    {
        // 重置某一条边的代价和容量
        auto resetCostOfEdge = [&](Edge &edge, int64_t cost, int64_t cap)
        {
            edge.cost = cost;
        ...
        for (int siteId = 0; siteId < numSites; ++siteId)
        {
            ...
            vector<int64_t> threshold({0,
                                       (int64_t)(sortedFlows[num95moments - 1].first * (1 - pow(0.9, epoch))),
            ...
            {
                int level = 0;
                for (Edge &edge : graph[siteId2GraphId[t][siteId]])
                    if (edge.isForward)
                    {
                        ++level;
                        ;
                        // if (!is5[t][siteId]) // 小于96; 分4条边
                        if (flows[t] <= sortedFlows[num95moments - 1].first) // 小于96; 分4条边
    ...
    void run()
    {
        ...
        for (int epoch = 0; epoch < 50; epoch++)
        {
            ...
            cout << "Epoch Time Spent " << (clock() - epochClock) << endl;
        }
...
int main()
{
    ...
    else if (ifstream("../CodeCraft-2022/data/config.ini").good()) // linux测试环境
    {
        ...
        dataDir = "../CodeCraft-2022/pressure0.6_data";
        outputPath = "../CodeCraft-2022/pressure0.6_data/solution.txt";
    }
