            ...
            for (int &sid : sortedSiteIds)
            {
                ...
                for (int t = 0; t < numMoments; ++t)
                {
                    ...
                    for (int &uid : siteConnUserIds[sid])
                    {
                        ...
                        // 策略2：按比例接收用户需求(效果好/速度慢)
                        int64_t userSumSiteBand = 0;
                        for (int &insid : userConnSiteIds[uid])
                            userSumSiteBand += siteList.getBandwidth(insid) - sitesFlow[t][insid];
                        curSumFlow += demandInfo[t][uid] * (siteList.getBandwidth(sid) - sitesFlow[t][sid]) / userSumSiteBand;
                    }
