...
public:
    ...
    void greed7()
    {
        ...
        {
            // 检查任务是否结束
            int numOptionalSites = 0; // 未处理完成的边缘节点的剩余数量
            for (int s : sortedSiteIds)
                if (siteNumOptionalMoments[s] > 0)
                    numOptionalSites++;
            if (numOptionalSites == 0)
                break;
            // 全局统计潜力值,挑选5%节点
            /**
             * 这里收集节点时，改成优先队列，可以优化效率
             *
             * */
            static vector<pair<int64_t, PII>> Qlist; // <flow,<t,s>>
            Qlist.clear();
            uint32_t numNodeToBeChose = numSites; // todo
            for (int &s : sortedSiteIds)
            {
                if (siteNumOptionalMoments[s] == 0)
                    continue;
                for (int t = 0; t < numMoments; ++t)
                {
                    ...
                    Qlist.push_back(pair<int64_t, PII>(abs(potential[t][s] - siteList.getBandwidth(s)), PII(t, s))); // 策略2
                }
            }
            sort(Qlist.begin(), Qlist.end()); // 所有的潜力值排序,选出前5%
            static vector<PII> QlistSelected;
            static unordered_set<int> selectedMoments; // 选中的时间
            QlistSelected.clear();
            selectedMoments.clear();
            for (uint32_t i = 0; i < Qlist.size(); i++)
            {
                if (siteNumOptionalMoments[Qlist[i].second.second] > 0 && QlistSelected.size() < numNodeToBeChose)
                {
                    siteNumOptionalMoments[Qlist[i].second.second]--;
                    QlistSelected.push_back(Qlist[i].second);
                    selectedMoments.insert(Qlist[i].second.first);
                }
            }
            // 选出了若干时刻节点，将其拉满 **********************************************************
            for (PII ts : QlistSelected)
            {
                int bestMoment = ts.first;
                int sid = ts.second;
