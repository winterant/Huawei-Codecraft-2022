...
public:
    ...
    void greed5()
    {
        ...
        int numOptionalSites = sortedSiteIds.size();
        while (numOptionalSites)
        {
            for (int &sid : sortedSiteIds)
            {
                if (siteNumOptionalMoments[sid] == 0)
                    continue;
                uint32_t numMomentsToBeChose = min(siteNumOptionalMoments[sid], num5moments / 5); // 每次直接找1%个最大节点，可以优化2w
                siteNumOptionalMoments[sid] -= numMomentsToBeChose;
                if (siteNumOptionalMoments[sid] == 0)
                    --numOptionalSites;
                // 当前节点选出numMomentsToBeChose个最优时刻
                priority_queue<PLI, vector<PLI>, greater<PLI>> Qband;
                // int bestMoment = -1;
