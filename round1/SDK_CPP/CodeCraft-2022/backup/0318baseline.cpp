...
public:
    int run(int epochs = 10)
    {
         ...
         * 核心思路：
         * 1.先任意初始化一个可行解（例如贪心）
         * 2.对于一个边缘节点s，按带宽值将时间序列排序，记第95%时刻为t，其流量值为flow
         * 2.1 t时刻重新规划，使用费用流，95位代价大，05位代价小。但是如何避免陷入局部最优？
         * 3.精调。此时95位已固定，如何进一步降低95位，提高05位？
         **/
        // 1. 初始化一个可行解
        vector<PII> totalFlowOfsite(numSites, PII(0, 0));
        for (int s = 0; s < numSites; ++s)
        {
            totalFlowOfsite[s].second = s;
            for (int u = 0; u < numUsers; ++u)
            {
                if (qos.get(s, u) >= qosConstraint)
                    continue;
                for (int t = 0; t < numMoments; ++t)
                {
                    totalFlowOfsite[s].first += demand.getDemand(t, u);
                }
            }
        }
        sort(totalFlowOfsite.begin(), totalFlowOfsite.end());
        reverse(totalFlowOfsite.begin(), totalFlowOfsite.end());
        for (int t = 0; t < numMoments; ++t)
        {
            // 当前时刻的用户请求
            ...
            // 每个边缘节点收集请求
            for (auto &pair : totalFlowOfsite)
            {
                int siteId = pair.second;
