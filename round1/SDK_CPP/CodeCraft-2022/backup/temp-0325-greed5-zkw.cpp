...
public:
    ...
    const string &getSiteName(int siteId)
    {
...
public:
    ...
    const string &getUserName(int userId)
    {
...
public:
    GraphFlow(SiteList &siteList, Demand &demand, Qos &qos, int qosConstraint)
    ...
    void buildGraph(int64_t defaultCost = 1)
    {
        for (int t = 0; t < numMoments; ++t)
        {
            ...
            for(int s=0;s<numSites;++s)
            {
                for(int i=0;i<numSiteEdge;++i) // 拆边
    ...
    void initEdgeCost(int epoch)
    {
        for (int s = 0; s < numSites; ++s)
        {
            ...
            for (int t = 0; t < numMoments; ++t)
            {
                sortedFlows[t].first = sitesFlow[t][s];
                sortedFlows[t].second = t;
            }
            sort(sortedFlows.begin(), sortedFlows.end());
            // 按95位设定阈值
            vector<int64_t> threshold({sortedFlows[num95moments - 1].first * 40 / 100,
                                           sortedFlows[num95moments - 1].first * 70 / 100,
            ...
            for (uint32_t i = 1; i < threshold.size(); ++i) // 根据阈值拆分容量
                threshold[i] -= threshold[i - 1];
            // 修改边代价
            for(int ti=0;ti<numMoments;++ti)
            {
                int64_t flow=sortedFlows[ti].first;
                int moment = sortedFlows[ti].second;
                int level = 0;
                for (Edge &edge : graph[siteId2GraphId[moment][s]])
                    if (edge.isForward)
                    {
                        ++level;
                        if(ti<num95moments) // 属于95边，按阈值分配
                            resetCostOfEdge(edge, level, threshold[level - 1]);
                        else // 5%节点
                        {
                            if (level == 1)
                                resetCostOfEdge(edge, 0, siteList.getBandwidth(s));
                            else
    ...
    void reassignCostOfSite(int epoch)
    {
        ...
        for (int siteId = 0; siteId < numSites; ++siteId)
        {
            ...
            sort(sortedFlows.begin(), sortedFlows.end());
            // 设定阈值，指定代价
            double composeLower = 1 - pow(0.9, epoch+10); // (1-adjustRange -> 1.0) 影响巨大
            double composeUpper = 1 + pow(0.9, epoch+10);  // 影响微小
...
public:
            ...
            for (int &sid : sortedSiteIds)
            {
                ...
                for (int t = 0; t < numMoments; ++t)
                {
                    ...
                    for (int &uid : siteConnUserIds[sid])
                    {
                        ...
                        // 策略2：按比例接收用户需求(效果好)
                        curSumFlow += demandInfo[t][uid] * (siteList.getBandwidth(sid) - sitesFlow[t][sid]) / userSumAllowedBandwidth[t][uid];
                    }
                ...
                for (int &uid : siteConnUserIds[sid]) // 刷新用户可连接带宽余量总和
    ...
    void buildGraph(int64_t defaultCost = 1)
    {
        ...
        for (int t = 0; t < numMoments; ++t)
        {
            ...
            for (int &s : sortedSiteIds)
            {
                ...
                if (!is5[t][s]) // 普通节点拆边
                {
                    vector<int64_t> flows({siteTimedFlow[s][num95moments - 1].first * 40 / 100,
    ...
    void reassignCostOfSite(int epoch)
    {
        ...
        for (int siteId = 0; siteId < numSites; ++siteId)
        {
            ...
            // 设定阈值，指定代价 todo
            double composeLower = 1 - pow(0.9, epoch+1); // (1-adjustRange -> 1.0) 影响巨大
