...
public:
    ...
    void greed6()
    {
        ...
        // 下面进行分组查找
        vector<int> timeRange(numMoments);
        for (int t = 0; t < numMoments; ++t)
            timeRange[t] = t;
        vector<int> candidateSites(sortedSiteIds);                 // 候选边缘节点
        vector<vector<int>> candidateMoments(numSites, timeRange); // 每个边缘节点候选时刻
        int groupStart = 0, groupSize = 40;
        for (int epoch = 0; epoch < num5moments * numSites && !candidateSites.empty(); ++epoch)
        {
            // 全局挑选出最优状态点
            PII bestStatusIndex = PII(-1, -1);
            int64_t bestStatusFlow = -1;
            for (uint32_t i = groupStart, gs = groupSize; gs--; i = (i + 1) % candidateSites.size()) // 边缘节点
            {
                if (candidateMoments[candidateSites[i]].size() <= (uint32_t)num95moments) // 边缘节点已找到5%个节点，无需再找
                    continue;
                for (uint32_t j = 0; j < candidateMoments[candidateSites[i]].size(); ++j) // 时刻
                {
                    int sid = candidateSites[i];
                    int moment = candidateMoments[sid][j];
                    ...
                    for (int &uid : siteConnUserIds[sid])
                    {
                        ...
                        // 策略2：按比例接收用户需求(效果好)
                        curSumFlow += demandInfo[moment][uid] * (siteList.getBandwidth(sid) - sitesFlow[moment][sid]) / userSumAllowedBandwidth[moment][uid];
                    }
                    // curSumFlow = min(curSumFlow, siteList.getBandwidth(sid) - sitesFlow[t][sid]); // 加上这句分数爆炸
                    if (bestStatusFlow < curSumFlow)
                    {
                        bestStatusFlow = curSumFlow;
                        bestStatusIndex = PII(i, j);
                    }
                }
            }
            // 挑选好状态了，检查一下是否已无解
            if (bestStatusFlow < 0)
                break;
            // 选中的节点
            int &sid = candidateSites[bestStatusIndex.first];
            int &moment = candidateMoments[sid][bestStatusIndex.second];
            if (bestStatusFlow > 0)
            {
                // 挑选好状态了，开始分发流量
                ...
                // 刷新用户可连接带宽余量总和
                for (int &uid : siteConnUserIds[sid])
                    userSumAllowedBandwidth[moment][uid] -= incrementFlow;
            }
            // 更新一下每个边缘节点的候选时刻，被用过的时刻可以删掉
            swap(moment, candidateMoments[sid].back());
            candidateMoments[sid].pop_back();
            // todo  优化trick：可以尝试删除候选边缘节点
            // if (candidateMoments[sid].size() <= (uint32_t)num95moments)
            // {
            //     swap(sid, candidateSites.back());
            //     candidateSites.pop_back();
            // }
            if (candidateSites.size())
                groupStart = (groupStart + 1) % candidateSites.size();
        }
    ...
    void buildGraph(int64_t defaultCost = 1)
    {
        ...
        for (int t = 0; t < numMoments; ++t)
        {
            ...
            for (int &s : sortedSiteIds)
            {
                ...
                if (!is5[t][s]) // 普通节点拆边
                {
                    vector<int64_t> flows({upperDiv(siteSum95Flow[s], num95moments) * 50 / 100,
                                           siteTimedFlow[s][num95moments - 1].first,
                                           siteTimedFlow[s][num95moments].first,
                                           bandwidth});
                    for (uint32_t i = 0; i < flows.size() - 1; ++i) // 调整，让阈值不要超过后面
                        flows[i] = min(flows[i], flows[i + 1]);
                    for (uint32_t i = 1; i < flows.size(); ++i) // 根据阈值拆分容量
                        flows[i] -= flows[i - 1];
    ...
    void reassignCostOfSite(int epoch)
    {
        ...
        for (int siteId = 0; siteId < numSites; ++siteId)
        {
            ...
            // 设定阈值，指定代价 todo
            double composeLower = 1 - pow(0.95, epoch); // (1-adjustRange -> 1.0) 影响巨大
